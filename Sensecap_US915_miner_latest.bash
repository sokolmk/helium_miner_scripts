...
date
echo "$0 starting with MINER=$MINER NET=$NET DATADIR=$DATADIR LOGDIR=$LOGDIR REGION=$REGION ARCH=$ARCH running_image=$running_image miner_latest=$miner_latest GA=$GA"
...
docker run -d --init --env REGION_OVERRIDE="$REGION" --device "$DEVICE"--restart always --net "$NET" --name "$MINER" $LOGMOUNT --mount type=bind,source="$DATADIR",target=/var/data --mount type=bind,source=/home/pi/docker_config/docker.config,target=/config/sys.config quay.io/team-helium/miner:"$miner_latest"
